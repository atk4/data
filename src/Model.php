...
class Model implements \ArrayAccess, \IteratorAggregate
{
    ...
    public function containsOne($link, $defaults = [])
    {
        ...
        if (!is_array($defaults)) {
            $sub_model = $defaults ?: 'Model_'.$link;
        } elseif (isset($defaults[0])) {
            $sub_model = $defaults[0];
            unset($defaults[0]);
        } else {
            throw new Exception(['Model is not set for containsOne method', 'model' => get_class($this)]);
        }

        // first let's add hidden system field which will store this sub-model data
        $this->addField($link, ['type' => 'array', 'system' => true, 'serialize' => 'json']);

        // then define custom reference and hooks
        $this->addRef($link, function ($self) use ($link, $sub_model, $defaults) {

            // model should be loaded
            if (!$self->loaded()) {
                throw new \atk4\core\Exception(['Model should be loaded!', 'model'=>get_class($self)]);
            }

            // create sub model object
            $sub_model = $self->factory($sub_model, $defaults);
            $sub_model->id_field = null;
    ...
    public function containsMany($link, $defaults = [])
    {
        ...
        if (!is_array($defaults)) {
            ...
            throw new Exception(['Model is not set for containsMany method', 'model' => get_class($this)]);
        }
