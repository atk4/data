...
class ModelAggregateTest extends TestCase
{
    ...
    public function testAggregateFieldExpressionSql(): void
    {
        ...
        $this->assertSameSql(
            'select sum(`amount`) `xyz`, sum(`order`) `sum_hasone` from (select `invoice`.`id`, `invoice`.`client_id`, `invoice`.`name`, `invoice`.`amount`, (select `name` from `client` `_c_2bfe9d72a4aa` where `id` = `invoice`.`client_id`) `client`, (select `order` from `client` `_c_2bfe9d72a4aa` where `id` = `invoice`.`client_id`) `order` from `invoice`) `_tm` group by `abc`',
            $aggregate->action('select')->render()[0]
        );
    }
}
